...
class ViewController {
	 ...
	 * @codeCoverageIgnore
	 */
	private static function addJsonHeaders($status)
	{
		header(sprintf("HTTP/1.1 %s %s", $status, HttpStatus::getStatus($status)));
		header("Content-type: application/json");
	}

	public static function sendJson($ret, $status = 200, $addHeaders = true)
	{
		if ($addHeaders)
		{
	...
	protected function __construct($view = null, array $vars = [])
	{
		$this->vars = $vars;
		$this->view = $view;
	}
	...
	protected function parseViewFile($file, &$vars) {
		if(!file_exists($file)) {
			return FALSE;
		}
		else {
	...
	protected static function esc_html_recursive( $data = FALSE ) {
